...
- Added [`agrosintesis()`](../reference/agrosintesis.md), a high-level
  wrapper to automate multi-variable ANOVA summaries using
  [`agrobox()`](../reference/agrobox.md) as the core engine.
  ...
  - When `estructura = NULL`, the analysis is performed globally (no
    clustering).
- Added [`agroexcel()`](../reference/agroexcel.md) to export agrobox and
  agrosintesis results to Excel (`.xlsx`) files.
  ...
  - Sheet names are sanitized to comply with Excel and Windows
    limitations.
- Added [`agrotabla()`](../reference/agrotabla.md) to export result
  tables as high-resolution PNG images.
...
- Cluster labels now preserve original factor levels instead of numeric
  codes.
...
- Improved CRAN compliance by:
  ...
  - Avoiding [`library()`](https://rdrr.io/r/base/library.html) calls
    inside functions.
